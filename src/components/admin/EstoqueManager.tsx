...
export const EstoqueManager = ({
  ...
  const fetchFuncionarios = async () => {
    ...
    try {
      ...
      console.log('Resposta do Supabase funcionários (EstoqueManager):', {
        data,
        error
      });
      if (error) {
  ...
  const handleDeleteFuncionario = async (funcionario: Funcionario) => {
    try {
      const {
        error
      } = await supabase.from('funcionarios').delete().eq('id', funcionario.id);
      if (error) {
  ...
  const handleDeleteFerramenta = async (ferramenta: Ferramenta) => {
    try {
      const {
        error
      } = await supabase.from('ferramentas').delete().eq('id', ferramenta.id);
      if (error) {
  ...
  const handleReserva = async () => {
    ...
    try {
      ...
      const {
        error
      } = await supabase.from('ferramentas').update(updateData).eq('id', reservandoFerramenta.id);
      if (error) {
  ...
  const filteredMateriais = materiais.filter(material => material.nome.toLowerCase().includes(searchTerm.toLowerCase()) || material.tag.toString().includes(searchTerm) || material.unidade?.toLowerCase().includes(searchTerm.toLowerCase())).sort((a, b) => a.nome.localeCompare(b.nome));
  const filteredFerramentas = ferramentas.filter(ferramenta => ferramenta.nome.toLowerCase().includes(searchTerm.toLowerCase()) || ferramenta.categoria.toLowerCase().includes(searchTerm.toLowerCase()) || ferramenta.tag.toLowerCase().includes(searchTerm.toLowerCase())).sort((a, b) => a.nome.localeCompare(b.nome));
  const filteredFuncionarios = funcionarios.filter(funcionario => funcionario.nome.toLowerCase().includes(searchTerm.toLowerCase()) || funcionario.matricula.toString().includes(searchTerm) || funcionario.setor.toLowerCase().includes(searchTerm.toLowerCase())).sort((a, b) => a.nome.localeCompare(b.nome));
      ...
      <CardContent className="pt-0">
        <Tabs value={currentTab} onValueChange={setCurrentTab} className="space-y-4">
          <TabsList className="grid w-full grid-cols-3 h-10">
            <TabsTrigger 
              value="materiais" 
              className="min-h-[36px] transition-colors duration-200"
              type="button"
            >
              Materiais
            </TabsTrigger>
              ...
              <div className="flex justify-end">
                <Dialog>
                  <DialogTrigger asChild>
                    <Button className="min-w-[180px] h-10">
                  ...
                  <DialogContent className="max-w-md">
                    ...
                    <div className="space-y-4">
                      ...
                      <Button className="w-full min-h-[40px]" onClick={handleAddMaterial} disabled={isAddingMaterial}>
                        {isAddingMaterial ? (
                    ...
                    return <TableRow key={material.id}>
                          ...
                          <TableCell>
                            <div className="flex gap-2">
                              ...
                              <Dialog open={dialogEntradaOpen && materialParaEntrada?.id === material.id} onOpenChange={open => {
                            ...
                            if (!open) {
                              ...
                              setQuantidadeEntrada("");
                                ...
                                <DialogContent className="max-w-md">
                                  ...
                                  {materialParaEntrada && materialParaEntrada.id === material.id && <div className="space-y-4">
                                      <div>
                                      ...
                                      <div>
                                        ...
                                        <Input id="quantidade-entrada" type="number" min="1" value={quantidadeEntrada} onChange={e => setQuantidadeEntrada(e.target.value)} placeholder={`Ex: 10 ${material.unidade}`} />
                                      </div>
                                      
                                      <Button className="w-full min-h-[40px]" onClick={handleAddEntrada} disabled={isAddingEntrada}>
                                        {isAddingEntrada ? 'Adicionando...' : `Adicionar ${quantidadeEntrada || '0'} ${material.unidade} ao estoque`}
                                      </Button>
                                    </div>}
                                </DialogContent>
                              </Dialog>
                            ...
                            if (!open) setEditingMaterial(null);
                                ...
                                <DialogContent>
                                  ...
                                  {editingMaterial && editingMaterial.id === material.id && <div className="space-y-4">
                                    ...
                                    <Button className="w-full min-h-[40px]" onClick={handleEditMaterial}>
                                      Salvar Alterações
                                    </Button>
                                  </div>}
                                </DialogContent>
                              ...
                              <AlertDialog>
                                ...
                                <AlertDialogContent>
                                  ...
                                  <AlertDialogFooter>
                                    ...
                                    <AlertDialogAction onClick={() => handleDeleteMaterial(material)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                                      Excluir Material
                                    </AlertDialogAction>
                                  </AlertDialogFooter>
                                </AlertDialogContent>
                ...
                <Dialog>
                  ...
                  <DialogContent className="max-w-md">
                    ...
                    <div className="space-y-4">
                      ...
                      })} rows={4} placeholder="Ex: Cor: Preta, Tensão: 220v, Potência: 500W" />
                        <p className="text-xs text-muted-foreground mt-1">
                          Digite as características separadas por vírgula
                        </p>
                      </div>
                      <Button className="w-full min-h-[40px]" onClick={handleAddFerramenta} disabled={isAddingFerramenta}>
                        {isAddingFerramenta ? (
                          <span className="flex items-center gap-2">
                            <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />
                            Adicionando...
                    ...
                    return <TableRow key={ferramenta.id}>
                          ...
                          if (!open) {
                            ...
                            setMatriculaReserva("");
                              ...
                              <DialogTrigger asChild>
                                <Button size="sm" variant={ferramenta.reserva ? "destructive" : "outline"} onClick={() => {
                              ...
                              <DialogContent className="max-w-md">
                                ...
                                {reservandoFerramenta && reservandoFerramenta.id === ferramenta.id && <div className="space-y-4">
                                    <div>
                                      <p className="text-sm text-muted-foreground mb-2">
                                    ...
                                    {ferramenta.reserva ? <div className="space-y-2">
                                        <p className="text-sm">
                                        ...
                                        <Input id="matricula-reserva" value={matriculaReserva} onChange={e => setMatriculaReserva(e.target.value)} placeholder="Digite a matrícula" />
                                      </div>}
                                    
                                    <Button className="w-full min-h-[40px]" variant={ferramenta.reserva ? "destructive" : "default"} onClick={handleReserva} disabled={isProcessingReserva}>
                                      {isProcessingReserva ? 'Processando...' : ferramenta.reserva ? 'Cancelar Reserva' : 'Confirmar Reserva'}
                                    </Button>
                                  </div>}
                              </DialogContent>
                            </Dialog>
                                ...
                                <AlertDialogContent>
                                  <AlertDialogHeader>
                                    ...
                                    <AlertDialogDescription>
                                      ...
                                      {ferramenta.reserva && <span className="block text-yellow-600 font-semibold mt-1">
                                  ...
                                  <AlertDialogFooter>
                                    <AlertDialogCancel>Cancelar</AlertDialogCancel>
                                    <AlertDialogAction onClick={() => handleDeleteFerramenta(ferramenta)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                                      Excluir Ferramenta
                                    </AlertDialogAction>
                                  </AlertDialogFooter>
                                </AlertDialogContent>
                ...
                <Dialog>
                  ...
                  <DialogContent className="max-w-md">
                    ...
                    <div className="space-y-4">
                      ...
                      <Button className="w-full min-h-[40px]" onClick={handleAddFuncionario} disabled={isAddingFuncionario}>
                        {isAddingFuncionario ? (
                          <span className="flex items-center gap-2">
                            <div className="w-4 h-4 border-2 border-current border-t-transparent rounded-full animate-spin" />
                            Adicionando...
                ...
                <Table>
                  <TableHeader>
                    <TableRow>
                      ...
                      <TableHead className="px-[50px]">Setor</TableHead>
                      <TableHead className="mx-0 px-[32px]">WhatsApp</TableHead>
                      <TableHead className="text-center">Em posse</TableHead>
                      <TableHead className="px-[36px]">Ações</TableHead>
                    </TableRow>
                  </TableHeader>
                  <TableBody>
                    {loadingFuncionarios ? <TableRow>
                        <TableCell colSpan={6} className="text-center">Carregando funcionários...</TableCell>
                      </TableRow> : filteredFuncionarios.length === 0 ? <TableRow>
                        <TableCell colSpan={6} className="text-center text-muted-foreground">
                          ...
                          <TableCell>
                            {funcionario.numero_whatsapp ? formatWhatsAppForDisplay(funcionario.numero_whatsapp) : 'Não informado'}
                          </TableCell>
                          <TableCell className="text-center">
                            ...
                            setEditFuncionarioDialogOpen(true);
                              ...
                              <DialogContent>
                                ...
                                numero_whatsapp: e.target.value
                                    ...
                                    <Button className="w-full min-h-[40px]" onClick={handleEditFuncionario}>
                                      Salvar Alterações
                                    </Button>
                                  </div>}
                              </DialogContent>
                              ...
                              <AlertDialogContent>
                                <AlertDialogHeader>
                                  ...
                                  <AlertDialogDescription>
                                    ...
                                    {funcionario.posse_ferramentas.length > 0 && <span className="block text-yellow-600 font-semibold mt-1">
                                ...
                                <AlertDialogFooter>
                                  <AlertDialogCancel>Cancelar</AlertDialogCancel>
                                  <AlertDialogAction onClick={() => handleDeleteFuncionario(funcionario)} className="bg-destructive text-destructive-foreground hover:bg-destructive/90">
                                    Excluir Funcionário
                                  </AlertDialogAction>
                                </AlertDialogFooter>
                              </AlertDialogContent>
